		...
		if "://" in item["path"]:
			...
			# Kiểu link .ts
				...
				item["path"] = "plugin://plugin.video.f4mTester/?url=%s&streamtype=TSDOWNLOADER&use_proxy_for_chunks=True&name=%s" % (
					...
					urllib.parse.quote_plus(item["label"])
				)
				item["path"] = pluginrootpath + \
					"/executebuiltin/" + urllib.parse.quote_plus(item["path"])
			# Kiểu direct link
			else:
				if "acestream" in item["path"]:
					item["label"] = "[AceStream] %s" % item["label"]
				item["path"] = pluginrootpath + \
					"/play/%s" % urllib.parse.quote_plus(item["path"])
				item["is_playable"] = True
		else:
			# Nếu không phải...
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				...
				tmp[-1] = urllib.parse.quote_plus(tmp[-1])
			...
			if "spreadsheets/d/" in item["path"]:
				...
				item["path"] = "plugin://plugin.video.xshare/?mode=38&page=0&url=" + \
					urllib.parse.quote_plus(item["path"])
				...
				item["path"] = "plugin://plugin.video.xshare/?mode=3&page=0&url=" + \
					urllib.parse.quote_plus(item["path"])
				item["is_playable"] = True
				item["path"] = pluginrootpath + "/play/" + urllib.parse.quote_plus(item["path"])
				...
				item["path"] = "plugin://plugin.video.kodi4vn.launcher/showimage/%s/" % urllib.parse.quote_plus(
					item["path"])
			elif re.search("\.ts$", item["path"]):
				item["path"] = "plugin://plugin.video.f4mTester/?url=%s&streamtype=TSDOWNLOADER&use_proxy_for_chunks=True&name=%s" % (
					urllib.parse.quote(item["path"]),
		...
		if item["label2"].startswith("http"):
			item["path"] += "?sub=" + urllib.parse.quote_plus(item["label2"].encode("utf8"))
		items += [item]
	if url_path == "0":
		add_playlist_item = {
			"context_menu": [
...
def ClearPlaylists(item=""):
	...
	return (label, actions.background(
		"%s/remove-playlists/%s" % (pluginrootpath, urllib.parse.quote_plus(item))
	))
...
def AceList(path="0", tracking_string="AceList"):
	...
	for _time, _date, sport, aceurl, title in match:
		...
		item["path"] = "%s/play/%s/%s" % (
			pluginrootpath,
			urllib.parse.quote_plus(aceurl),
			urllib.parse.quote_plus("[AceList] %s" % item["label"])
		)
		item["is_playable"] = True
		items += [item]
	return plugin.finish(items)
	...
	for i in fshare_items:
		...
		if not i["type"]:  # is folder
			item["path"] = "%s/fshare/%s/%s" % (
				pluginrootpath,
				urllib.parse.quote_plus("https://www.fshare.vn/folder/" + i["linkcode"]),
				urllib.parse.quote_plus("[FShare] %s" % name)
			)
			item["label"] = "[FShare] %s" % name
		else:
			item["path"] = "%s/play/%s/%s" % (
				pluginrootpath,
				urllib.parse.quote_plus("https://www.fshare.vn/file/" + i["linkcode"]),
				urllib.parse.quote_plus("[FShare] %s (%s)" % (name, size))
			)
	...
	if len(fshare_items) >= 20:
		...
		items.append({
			...
			'path': '%s/fshare/%s/%s' % (
				...
				urllib.parse.quote_plus(tracking_string)
			),
			...
			if ":/" not in item["label2"]:
				...
				if "version" in result and version_cmp(json_result["result"]["addon"]["version"], item["label2"]) >= 0:
					...
					try:
						...
						download(urllib.parse.unquote_plus(item["path"]), item["label"])
				...
				if not os.path.exists(xbmc.translatePath(item["label2"])):
					try:
						item["path"] = "http" + item["path"].split("http")[-1]
						download(urllib.parse.unquote_plus(item["path"]), item["label2"])
						installed += [item["label"].encode("utf-8")]
					except:
						failed += [item["label"].encode("utf-8")]
...
def RepoSection(path="0", tracking_string=""):
	...
	install_all_item = {
		...
		"path": pluginrootpath + "/install-repo/%s/%s" % (path, urllib.parse.quote_plus("Install all repo")),
		"is_playable": False,
...
def download(download_path, repo_id):
	...
	if ":/" not in repo_id:
		...
		urllib.request.urlretrieve(download_path, zipfile_path)
	...
	for item in items:
		if "plugin.video.thongld.vnplaylist" in item["path"]:
			...
			item["path"] = "%s/%s?%s" % (tmps[0],
			                             urllib.parse.quote_plus(item["label"]), tail)
	return items


@plugin.route('/showimage/<url>/<tracking_string>')
def showimage(url, tracking_string):
	xbmc.executebuiltin("ShowPicture(%s)" % urllib.parse.unquote_plus(url))
...
def execbuiltin(path, tracking_string=""):
	...
	try:
		xbmc.executebuiltin('XBMC.RunPlugin(%s)' % urllib.parse.unquote_plus(path))
	except:
		pass
...
def get_playable_url(url):
	if "youtube" in url:
		...
		try:
			...
			if url.endswith(".acelive"):
				url = "http://localhost:6878/ace/getstream?url=" + \
					urllib.parse.quote_plus(url) + "&.mp4"
		except:
			url = 'plugin://program.plexus/?url=%s&mode=1&name=P2PStream&iconimage=' % urllib.parse.quote_plus(
				url)
		...
		(resp, content) = http.request(
			oc_url,
			"POST", headers=h,
			body=urllib.parse.urlencode({'type': 'directLink', 'ip': ''})
		)
		...
		if os.path.exists(xshare_settings_path):
			with open(xshare_settings_path, "r") as f:
				...
				return "plugin://plugin.video.xshare/?mode=3&page=0&url=" + urllib.parse.quote_plus(url)
				# else:
...
def GetPlayLinkFromDriveID(drive_id):
	...
	try:
		...
		return play_url
	tail = "|User-Agent=%s&Cookie=%s" % (urllib.parse.quote(
		sheet_headers["User-Agent"]), urllib.parse.quote(resp['set-cookie']))
